...
const TodaysCases = () => {
  ...
  const { data, refetch, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await axiosSecure.get(
        `/cases/today?page=${page}&limit=${LIMIT}&search=${search}`,
      );
      return res.data;
    },
  ...
  const handleSavePDF = () => {
    ...
    doc.setFont("helvetica", "bold");
    doc.text(`TODAY CASES REPORT - PAGE ${page}`, 148, 38, {
      align: "center",
    });
  ...
  return (
    <div className="p-4">
      <h2 className="text-2xl font-bold mb-4">Today Cases</h2>
