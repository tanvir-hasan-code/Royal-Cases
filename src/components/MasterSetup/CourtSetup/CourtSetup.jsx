...
const CourtSetup = () => {
  ...
  const handleAdd = async (e) => {
    e.preventDefault();
    const name = e.target.court.value.trim();
    ...
    try {
      await axiosInstance.post("/courts", { name });
      toast.success("Court added!", { id: toastId });
      e.target.reset();
      refetch();
    } catch {
      toast.error("Failed to add court", { id: toastId });
    }
  };
  ...
  const handleUpdate = async (id) => {
    ...
    try {
      await axiosInstance.patch(`/courts/${id}`, {
        name: editValue,
      });
  ...
  const confirmDelete = async () => {
    ...
    try {
      await axiosInstance.delete(`/courts/${deleteId}`);
  ...
  return (
    <div className="bg-white p-6 rounded-xl shadow-md w-full max-w-3xl mx-auto">
      ...
      {isLoading ? (
        ...
        <p className="text-center text-gray-400">No court added yet</p>
      ) : (
